...
If you're using Claude Code, copy the command definitions to your local `.claude/commands/` directory:

```bash
cp .workflow/commands/*.md .claude/commands/
```

This enables the slash commands (`/nextspec`, `/spec`, `/design`, etc.) in your Claude Code session.

**Note**: `.claude/` is gitignored, so each developer sets up commands locally.
